    ...
    class User < Base
      class << self
        ...
        def get_profile_from_username(username)
          ...
          get("https://us-prof.np.community.playstation.net/userProfile/v1/users/#{username}/profile2?fields=npId,onlineId,accountId,avatarUrls,plus,aboutMe,languagesUsed,trophySummary(@default,level,progress,earnedTrophies),isOfficiallyVerified,personalDetail(@default,profilePictureUrls),personalDetailSharing,personalDetailSharingRequestMessageFlag,primaryOnlineStatus,presences(@default,@titleInfo,platform,lastOnlineDate,hasBroadcastData),requestMessageFlag,blocking,friendRelation,following,consoleAvailability")['profile']
        end
        ...
        def get_profile_from_account_id(account_id)
          get("https://m.np.playstation.com/api/userProfile/v1/internal/users/#{account_id}/profiles")
        end
      end
    end
  end
